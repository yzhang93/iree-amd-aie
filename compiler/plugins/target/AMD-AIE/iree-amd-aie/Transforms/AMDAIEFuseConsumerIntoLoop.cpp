...
void AMDAIEFuseConsumerIntoLoopPass::runOnOperation() {
  ...
  if (fuseDepth < 1) {
    funcOp->emitOpError("Invalid depth for fusing consumer.");
  ...
  for (unsigned depth = 1; depth <= fuseDepth; depth++) {
    ...
    // TODO(avarma): All the following combination seems to be giving the same
    //               results for an IR Walk :-
    //               1. PostOrder + ReverseIterator.
    //               2. PreOrder.
    //               3. PreOrder + ReverseIterator.
    //               Something while IR formation seems to be performing an ill
    //               operation maybe?
    loopBody->walk<WalkOrder::PostOrder, ReverseIterator>(
        [&](linalg::LinalgOp op) {
          if ((depth == 1 && linalg::isaContractionOpInterface(op)) ||
              (depth == 2 && !linalg::isaContractionOpInterface(op))) {
            linalgOp = op;
            return WalkResult::interrupt();
          }
          return WalkResult::advance();
    ...
    fusedConsumer->origConsumer->erase();
  }
}
