...
void AMDAIEPackAndTransposePass::runOnOperation() {
  ...
  linalg::LinalgOp elementwiseFusionOp;
  funcOp->walk<WalkOrder::PostOrder, ReverseIterator>([&](linalg::LinalgOp op) {
    if (isa<linalg::GenericOp>(op) &&
        isMatmulElementwiseFusion(cast<linalg::GenericOp>(op), linalgOp)) {
      elementwiseFusionOp = op;
  ...
  SmallVector<linalg::LinalgOp> opsForPacking = {linalgOp};
  if (elementwiseFusionOp) {
    opsForPacking.push_back(elementwiseFusionOp);
  }

  for (auto linalgOp : opsForPacking) {
    ...
    if (packOps.empty() || !packedOp || unpackOps.empty()) {
