...
bool isMatmulTransposeB(linalg::LinalgOp linalgOp) {
  ...
  if (isa<linalg::MatmulTransposeBOp, linalg::BatchMatmulTransposeBOp, linalg::Mmt4DOp>(
          linalgOp))
    return true;
  if (!isa<linalg::GenericOp>(linalgOp)) return false;
...
bool isMatmulProducerOfElementwise(linalg::LinalgOp linalgOp) {
  if (!linalg::isElementwise(linalgOp) || isa<linalg::FillOp, linalg::CopyOp>(linalgOp)) {
    return false;
  }
